...
namespace XBLMS.Core.Repositories
{
    ...
    {
        public async Task<List<ExamTmTree>> GetListAsync(AuthorityAuth auth)
        {
            var query = Q.NewQuery();
            if (auth.AuthType == Enums.AuthorityType.Admin || auth.AuthType == Enums.AuthorityType.AdminCompany)
            {
                query.Where(nameof(ExamTmTree.CompanyId), auth.CurManageOrganId);
            }
            else if (auth.AuthType == Enums.AuthorityType.AdminDepartment)
            {
                query.Where(nameof(ExamTmTree.DepartmentId), auth.CurManageOrganId);
            }
            else
            {
                query.Where(nameof(ExamTmTree.CreatorId), auth.AdminId);
            }
            return await _repository.GetAllAsync(query.OrderBy(nameof(ExamTmTree.Id)));
