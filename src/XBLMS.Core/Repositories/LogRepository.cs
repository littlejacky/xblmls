...
namespace XBLMS.Core.Repositories
{
    public class LogRepository : ILogRepository
    {
        ...
        private Query GetAdminQuery(AuthorityAuth auth, List<int> adminIds, string keyword, string dateFrom, string dateTo)
        {
            var query = Q
            ...
            if (auth.AuthType == AuthorityType.AdminCompany || auth.AuthType == AuthorityType.Admin)
            {
                ...
                {
                    query.WhereIn(nameof(Log.CompanyId), auth.CurManageOrganIds);
                }
            }
            if (auth.AuthType == AuthorityType.AdminDepartment)
            {
                query.WhereIn(nameof(Log.DepartmentId), auth.CurManageOrganIds);
            }
            if (auth.AuthType == AuthorityType.AdminSelf)
            {
                query.Where(nameof(Log.AdminId), auth.AdminId);
            }


            if ((adminIds == null || adminIds.Count == 0) && string.IsNullOrEmpty(keyword) && string.IsNullOrEmpty(dateFrom) && string.IsNullOrEmpty(dateTo))
            {
                return query;
            }
        ...
        public async Task<int> GetAdminLogsCountAsync(AuthorityAuth auth, List<int> adminIds, string keyword, string dateFrom, string dateTo)
        {
            return await _repository.CountAsync(GetAdminQuery(auth, adminIds, keyword, dateFrom, dateTo));
        }

        public async Task<List<Log>> GetAdminLogsAsync(AuthorityAuth auth, List<int> adminIds, string keyword, string dateFrom, string dateTo, int offset, int limit)
        {
            var query = GetAdminQuery(auth, adminIds, keyword, dateFrom, dateTo);
            query.Offset(offset).Limit(limit);
            return await _repository.GetAllAsync(query);
        }

        private Query GetUserQuery(AuthorityAuth auth, int userId, string keyword, string dateFrom, string dateTo)
        {
            var query = Q
        ...
        public async Task<int> GetUserLogsCountAsync(AuthorityAuth auth, int userId, string keyword, string dateFrom, string dateTo)
        {
            return await _repository.CountAsync(GetUserQuery(auth, userId, keyword, dateFrom, dateTo));
        }

        public async Task<List<Log>> GetUserLogsAsync(AuthorityAuth auth, int userId, string keyword, string dateFrom, string dateTo, int offset, int limit)
        {
            var query = GetUserQuery(auth, userId, keyword, dateFrom, dateTo);
            query.Offset(offset).Limit(limit);
            return await _repository.GetAllAsync(query);
        }
