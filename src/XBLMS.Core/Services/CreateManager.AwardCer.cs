        ...
        public async Task AwardCer(ExamPaper paper, int startId, int userId)
        {
            ...
            if (user != null && paper != null)
            {
                if (paper.CerId > 0)
                {
                    ...
                    if (cerInfo != null)
                    {
                        if (!await _databaseManager.ExamCerUserRepository.ExistsAsync(userId, paper.Id, examStar.PlanId, examStar.CourseId))
                        {
                            var cerId = await _databaseManager.ExamCerUserRepository.InsertAsync(new ExamCerUser
                            {
                                PlanId = examStar.PlanId,
                                CourseId = examStar.CourseId,
