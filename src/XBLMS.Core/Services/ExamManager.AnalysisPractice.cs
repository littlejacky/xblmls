...
namespace XBLMS.Core.Services
{
    ...
    {
        public async Task<(int answerTmTotal, double answerPercent, int allTmTotal, double allAnswerPercent, int collectTmTotal, double collectAnswerPercent, int wrongTmTotal, double wrongAnswerPercent)> AnalysisPractice(int userId, int companyId)
        {
            var tmGroups = await _examTmGroupRepository.GetListWithoutLockedAsync(companyId);
            ...
            if (tmGroups != null && tmGroups.Count > 0)
            {
                foreach (var tmGroup in tmGroups)
                {
                    if (tmGroup.OpenUser)
                    {
                        var total = await _examTmRepository.GetCountAsync(companyId, tmGroup);
                        allTmTotal += total;
                    }
                }
            }
