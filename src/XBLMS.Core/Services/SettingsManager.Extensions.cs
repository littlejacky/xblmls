...
namespace XBLMS.Core.Services
{
    public partial class SettingsManager
    {
        ...
        public List<Menu> GetMenus(bool isAdmin = false, bool isAuth = false, List<string> menuIds = null)
        {
            var section = Configuration.GetSection("extensions:menus");
            return GetMenus(section, isAdmin, isAuth, menuIds);
        }
        public string GetPermissionId(string menuId, string menuPermissionType)
        {
            return $"{menuId}_{menuPermissionType}";
        }
        private List<Menu> GetMenus(IConfigurationSection section, bool isAdmin = false, bool isAuth = false, List<string> menuIds = null)
        {
            ...
            if (section.Exists())
            {
                ...
                if (children != null)
                {
                    foreach (var child in children)
                    {
                        ...
                        if (!isAdmin && !menu.IsPermission) continue;
                        ...
                        else
                        {
                            if (menuIds == null)
                            {
                                menus.Add(new Menu
                                {
                                    ...
                                    Order = menu.Order,
                                    Target = menu.Target,
                                    Children = GetMenus(childSection,isAdmin, isAuth)
                                });
                            }
                            else
                            {
                                if (menuIds.Contains(menu.Id))
                                {
                                    menus.Add(new Menu
                                    {
                                        ...
                                        Order = menu.Order,
                                        Target = menu.Target,
                                        Children = GetMenus(childSection, isAdmin, isAuth, menuIds)
                                    });
                                }
