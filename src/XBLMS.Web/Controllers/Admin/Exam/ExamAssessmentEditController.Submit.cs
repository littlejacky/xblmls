        ...
        public async Task<ActionResult<BoolResult>> Submit([FromBody] GetSubmitRequest request)
        {
            ...
            if (assInfo.Id > 0)
            {
                ...
                if (request.SubmitType == SubmitType.Submit)
                {
                    ...
                    await _examManager.ArrangerExamAssessment(auth, assInfo);
            ...
            {
                assInfo.SubmitType = request.SubmitType;
                assInfo.CompanyId = auth.CompanyId;
                assInfo.CreatorId = auth.AdminId;
                assInfo.DepartmentId = auth.DepartmentId;
