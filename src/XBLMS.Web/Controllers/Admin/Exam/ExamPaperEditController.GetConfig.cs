...
namespace XBLMS.Web.Controllers.Admin.Exam
{
    public partial class ExamPaperEditController
    {
        ...
        public async Task<ActionResult<GetConfigResult>> GetConfig([FromBody] GetConfigRequest request)
        {
            ...
            {
                var count1 = await _examTmRepository.GetCountAsync(auth, hasGroup, allTm, tmIds, tx.Id, 1);
                var count2 = await _examTmRepository.GetCountAsync(auth, hasGroup, allTm, tmIds, tx.Id, 2);
                var count3 = await _examTmRepository.GetCountAsync(auth, hasGroup, allTm, tmIds, tx.Id, 3);
                var count4 = await _examTmRepository.GetCountAsync(auth, hasGroup, allTm, tmIds, tx.Id, 4);
                var count5 = await _examTmRepository.GetCountAsync(auth, hasGroup, allTm, tmIds, tx.Id, 5);
