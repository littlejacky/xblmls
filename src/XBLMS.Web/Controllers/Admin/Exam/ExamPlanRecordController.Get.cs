...
{
    public partial class ExamPlanRecordController
    {
        [HttpGet, Route(Route)]
        public async Task<ActionResult<GetResult>> GetManage([FromQuery] GetRequest request)
        {
            ...
            var (total, list) = await _examPlanRecordRepository.GetListAsync(auth, treeIds, request.Keyword, request.PageIndex, request.PageSize);
            //if (total > 0)
            //{
