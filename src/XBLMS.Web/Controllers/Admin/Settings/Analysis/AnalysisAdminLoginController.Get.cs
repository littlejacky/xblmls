...
namespace XBLMS.Web.Controllers.Admin.Settings.Analysis
{
    public partial class AnalysisAdminLoginController
    {
        ...
        public async Task<ActionResult<GetResult>> Get([FromBody] GetRequest request)
        {
            ...
            var successStats = await _statRepository.GetStatsAsync(auth, lowerDate, higherDate, StatType.AdminLoginSuccess);
            var failureStats = await _statRepository.GetStatsAsync(auth, lowerDate, higherDate, StatType.UserLogin);
