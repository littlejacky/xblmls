...
namespace XBLMS.Web.Controllers.Admin.Settings.Logs
{
    ...
    public partial class LogsAdminController : ControllerBase
    {
        ...
        public async Task<PageResult<Log>> GetResultsAsync(SearchRequest request)
        {
            ...
            var count = await _logRepository.GetAdminLogsCountAsync(auth, adminIds, request.Keyword, request.DateFrom, request.DateTo);
            var allLogs = await _logRepository.GetAdminLogsAsync(auth, adminIds, request.Keyword, request.DateFrom, request.DateTo, request.Offset, request.Limit);
