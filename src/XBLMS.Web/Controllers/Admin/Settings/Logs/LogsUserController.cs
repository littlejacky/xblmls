...
namespace XBLMS.Web.Controllers.Admin.Settings.Logs
{
    ...
    public partial class LogsUserController : ControllerBase
    {
        ...
        public async Task<PageResult<Log>> GetResultsAsync(SearchRequest request)
        {
            ...
            var count = await _logRepository.GetUserLogsCountAsync(auth, userId, request.Keyword, request.DateFrom, request.DateTo);
            var userLogs = await _logRepository.GetUserLogsAsync(auth, userId, request.Keyword, request.DateFrom, request.DateTo, request.Offset, request.Limit);
